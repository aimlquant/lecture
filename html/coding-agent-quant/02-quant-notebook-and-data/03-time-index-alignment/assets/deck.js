(function () {
  ...
  const requestedQaWidth = Number(params.get("qa-width"));
  const qaWidth = Number.isInteger(requestedQaWidth) && requestedQaWidth >= 320 && requestedQaWidth <= 720 ? requestedQaWidth : null;
  if (qaWidth) {
    document.documentElement.dataset.qaViewportWidth = String(qaWidth);
    document.documentElement.style.setProperty("--qa-viewport-width", `${qaWidth}px`);
  }
  ...
  function fitStage() {
    ...
    const viewportWidth = qaWidth || document.documentElement.clientWidth || window.innerWidth;
    ...
    if (qaWidth) {
      document.getElementById("deck").style.left = `calc(${qaWidth / 2}px - var(--deck-panel-shift, 0px))`;
    } else {
      document.getElementById("deck").style.removeProperty("left");
    }
  ...
  let tableNumber = 0;
  slides.forEach(slide => slide.querySelectorAll('table caption [data-deck-caption="table"]').forEach(label => { label.textContent = `표 ${++tableNumber}`; }));
  numberFigures();
  buildToc();
  fitStage();
