...
검토 공개는 세 단계다. `report`는 `report.html`만, 사용자가 발표자료 검토를 명시적으로
요청한 `slides`는 `report.html`과 `slides.html`만 `noindex`로 공개한다. `bundle`은 위의
다섯 파일 전체다. `slides` 단계에는 차시 인덱스·실습·대본·음성·영상이 없으며 완성 강의로
표시하지 않는다.
...
`slides` 단계부터, 실습 자료는 완성 번들이 생긴 뒤에만 연결한다.
